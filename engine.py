...
def finetune_one_epoch(args, student, teacher, masks, criterion, orig_criterion,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            original_loss_student = orig_criterion(samples, student_logits, targets, epoch, args.epochs) # added epoch and max_epoch
            ...
            if 1:
                ...
                if pruning_engine.pruning_iterations_done > 0:
                    if student.training and student_eval:
                        ...
                        #student.eval() #need to comment out to enable kd
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            if isinstance(out, tuple):
                if len(out) == 3:
                    x, x_dist, x_dist_dense = out
                    # collapse per-patch logits → [N, C]
                    x_dist_dense = x_dist_dense.mean(dim=1)
                    output = (x + x_dist + x_dist_dense) / 3
                elif len(out) == 2:
                    x, x_dist = out
                    output = (x + x_dist) / 2
                else:
                    # unexpected tuple length; just take first element
                    x = out[0]
                    output = x
            else:
                # single-output model
