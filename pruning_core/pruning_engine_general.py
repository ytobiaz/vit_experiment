...
class PruningConfigReader(object):
    ...
    def read_config(self, filename):
        ...
        self.read_field_value("layer_limits", []) 
...
class pytorch_pruning(object):
    def __init__(self, for_pruning_parameters, pruning_settings=dict(), log_folder=None, latency_regularization=0., latency_target=0., latency_look_up_table=""):
        ...
        # added lines (187-192)
        self.layer_limits = pruning_settings.get("layer_limits", [])
        # build a quick lookup: module_name → min_keep
        self._min_keep = {
        entry["module"]: entry["min_keep"]
        for entry in self.layer_limits
        }
...
#                                                                       mu=self.latency_regularization, latency=True)
        ...
        # added lines (841-824)
        layer_limits = getattr(self, "_min_keep", {})
        EMB_MIN = layer_limits.get("patch_embed.proj", None)
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            # added lines (894-900)
            if layer == 0 and EMB_MIN is not None:
                active = int(self.pruning_gates[layer].sum())
                allowed_zero = active - EMB_MIN
                zeroed_in_layer = 0
                # if we’re already at or below the floor, skip pruning emb entirely
                if allowed_zero <= 0:
                    continue
            ...
            for group in groups[layer]:
                if group[1] <= threshold_now:
                    ...
                    # added lines (909-914)
                    # embedding‐layer‐only check: don’t overshoot the floor
                    if layer == 0 and EMB_MIN is not None:
                        group_size = len(group[0])
                        if zeroed_in_layer + group_size > allowed_zero:
                            continue
                        zeroed_in_layer += group_size
