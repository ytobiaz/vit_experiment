...
__all__ = [
'deit_base_distilled_patch16_224','deit_base_distilled_patch16_224_small', 'regnety_160'
]
...
class DistilledVisionTransformer_small(VisionTransformer):
    def __init__(self, QK, V, MLP, head, *args, finetune=False, **kwargs):
...
class DistilledVisionTransformer(VisionTransformer):
    def __init__(self, *args, kernel_size=0, finetune=False, **kwargs):
        self.finetune = finetune
        kwargs.pop('pretrained_cfg', None)
        kwargs.pop('pretrained_cfg_overlay', None)
        super().__init__(*args, **kwargs)
...
# ============================================================================
# ---- begin RegNetY-160 integration ------------------------------------------------
# -------------------------------------------------------------------
# at top of your models.py, import the “real” Timm RegNet
from timm.models.regnet import RegNet as _TimmRegNet, default_cfgs as _timm_def, model_cfgs as _timm_mcfg
from timm.models.helpers import build_model_with_cfg, load_pretrained

# your existing filter for DeiT‐trained RegNetY-160
def _filter_fn(state_dict):
    return state_dict.get('model', state_dict)

# now subclass
class RegNetY160(_TimmRegNet):
    """Exact same weights & stages as Timm’s RegNetY-160, but with your custom forwards."""

    def __init__(self, pretrained=False, **kwargs):
        # grab the config dict for y_160
        cfg = _timm_mcfg['regnety_160']
        # note: _TimmRegNet expects __init__(cfg=..., **kwargs)
        super().__init__(cfg=cfg, **kwargs)
        # stash default_cfg so create_model(…, pretrained=True) still works
        self.default_cfg = _timm_def['regnety_160']
        # load the facebook/DeiT‐flavored pretrained weights if requested
        if pretrained:
            load_pretrained(
                self, 'regnety_160',
    ...
    def forward_features(self, x):
        # ——— paste in your exact forward_features from your Distilled code —
        # e.g. if yours was:
    ...
    def forward(self, x):
        # ——— paste in your exact forward from your Distilled code ——
...
# finally, register it under the same name
@register_model
def regnety_160(pretrained=False, **kwargs):
    # timm.create_model will pass in a pretrained_cfg arg—let’s pop it
    kwargs.pop('pretrained_cfg', None)
    kwargs.pop('pretrained_cfg_overlay', None)
    return RegNetY160(pretrained=pretrained, **kwargs)
# -------------------------------------------------------------------
